...
<template>
	<div id="drawer-right"
		...
		<header class="bg-slate-200 px-5 py-6 max-xl:p-4">
			<div class="flex justify-between max-sm:flex-col max-sm:gap-5">
				<div class="flex items-center gap-4">
					<CardsProfilePicture :image="currRecord?.image_url" @uploaded="imageUploadHandler($event)" />
					<div>
		...
		<div id="myTabContent">
			<template v-for="(tab, key) in content?.tab_headers" :key="key">
				<div v-if="tab.read" class="hidden p-4 rounded-lg dark:bg-gray-800" :id="`${key}`" role="tabpanel"
					...
					<SideDrawerTabsDetails v-else-if="tab.type === 'detail'" :key="currRecord" :data="currRecord"
						:headers="tab.fields" :canEdit="tab.write" @update-record="detailsUpdateHandler($event)" />

					<SideDrawerTabsHistory v-else-if="tab.type === 'history'" :key="currRecord" :data="currRecord"
						:canEdit="tab.write" :headers="content.headers" />

					<SideDrawerTabsPipeline v-else-if="tab.type === 'pipeline'" :key="currRecord" :data="currRecord"
						:pipelineField="tab.pipeline_field" :canEdit="tab.write"
						@update-record="detailsUpdateHandler($event)" />

					<SideDrawerTabsChips v-else-if="tab.type === 'chip'" :key="currRecord" :data="currRecord"
						:header="tab.field" :canEdit="tab.write" @update-record="detailsUpdateHandler($event)" />

					<SideDrawerTabsNotes v-else-if="tab.type === 'note'" :key="currRecord" :data="currRecord"
						:canEdit="tab.write" @update-record="detailsUpdateHandler($event)" />

					<SideDrawerTabsDocuments v-else-if="tab.type === 'document'" :key="currRecord" :data="currRecord"
						:canEdit="tab.write" @update-record="detailsUpdateHandler($event)" />

					<SideDrawerTabsAssign v-else-if="tab.type === 'assign'" :key="currRecord" :data="currRecord"
						:header="tab.field" :canEdit="tab.write" :store="toStore" />


					<div v-else class="flex flex-col gap-4">
						<div class="grid md:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-4">
							<div class="flex flex-col gap-2">
								<label for="sign-nda" class="block text-base font-medium text-gray-900 dark:text-white">Sign
									NDA</label>
								<select id="sign-nda" v-model="ndaRegion"
