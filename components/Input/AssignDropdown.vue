<template>
	<div class="relative">
		...
		<div
			...
			<ul v-if="options" class="p-3 text-sm text-gray-700 dark:text-gray-200">
				<template v-if="options?.length > 0">
					<li v-for="(opt, index) in options" :key="opt._id">
						<div class="flex rounded hover:bg-gray-100 dark:hover:bg-gray-600">
							<div class="flex items-center p-1">
								<input
									v-if="header.multi_select"
									:id="`option-${header.name}-${index}`"
									:name="`option-${header.name}-${index}`"
									type="checkbox"
									:value="opt"
...
<script setup>
	...
	const { header, fieldValue, prefix } = defineProps([
		"header",
		"fieldValue",
		"prefix",
	]);
	...
	watch(searchString, async (newVal, oldVal) => {
		...
		if (header) {
			...
			options.value = await store.findRecords([temp]);
		}
	});
