...
const { saveView, activeSaveView, saveViewList } = composable.useProject();
...
onBeforeMount(async () => {
	...
	for (const key in tableConfig.tab_headers) {
		if (tableConfig.tab_headers[key].type === 'custom') {
			tableConfig.tab_headers[key].read = true;
			tableConfig.tab_headers[key].write = true;

			const customLength = tableConfig.tab_headers[key].tabs.length;

			for (let i = 0; i < customLength; i++) {
				tableConfig.tab_headers[key].tabs[i].read = true;
				tableConfig.tab_headers[key].tabs[i].write = true;
			}

			// tableConfig.tab_headers[key].tabs[0].read = true;
			// tableConfig.tab_headers[key].tabs[0].write = true;

			console.log('tableConfig.tab_headers[key]', tableConfig.tab_headers[key])
			console.log('tableConfig.tab_headers[key].tabs[key]', tableConfig.tab_headers[key].tabs[0])
		}
		else {
...
<template>
	<div v-if="flag" class="flex flex-col h-full px-5 pt-5">
		<header class="mb-4">
			<div class="flex items-center justify-between max-xl:flex-col max-xl:items-start xl:gap-8 gap-4">
				<div class="flex xl:w-fit w-full items-center sm:gap-8 gap-4">
					<h2 v-if="store.recordsDataLength > 1" class="text-2xl max-sm:hidden sm:inline font-bold">
						{{ store.recordsDataLength }} Projects
					</h2>
					<h2 v-else-if="store.recordsDataLength <= 1" class="text-2xl max-sm:hidden sm:inline font-bold">
						{{ store.recordsDataLength }} Project
					</h2>
					<div class="flex gap-2">
						<!-- HIDE Dropdown -->
						<RecordsHideDropDown :tabHeaders="tableConfig.tab_headers" :headers="tableConfig.headers"
							:hideComposable="tableConfig.hideComposable" />

						<!-- SORT Dropdown -->

						<RecordsSortDropDown :key="saveView?.sort" :data="saveView?.sort" :tableConfig="tableConfig"
							@sort-data="sortHandler($event)" />

						<!-- FILTER Dropdown -->
						<RecordsFilterDropDown :key="saveView?.filters" :data="saveView?.filters" :tableConfig="tableConfig"
				...
				<div class="flex items-center xl:gap-8 gap-4 overflow-x-auto xl:w-fit w-full">
					...
					<div class="flex items-center gap-2">
						<button :id="`${tableConfig.tableName}-saveview-dropdown-button`"
							:data-dropdown-toggle="`${tableConfig.tableName}-saveview-dropdown`"
							class="primary-button !gap-4" type="button">
							<BookmarkIcon class="w-4 h-4 font-semibold md:hidden max-md:w-5 max-md:h-5" />
						...
						<!-- Dropdown menu -->
						<div :id="`${tableConfig.tableName}-saveview-dropdown`"
							...
							<div v-if="activeSaveView"
								class="flex justify-between items-center px-3 py-2 gap-2 text-gray-900 dark:text-white">
								<h2 class="font-medium">
									{{ activeSaveView?.name ? activeSaveView.name : "Save view" }}
								</h2>
								<button class="hover:bg-gray-200 rounded-full p-1" @click="clearSaveView">
									<XMarkIcon class="w-4 h-4" />
								</button>
							</div>
							<ul v-if="saveViewList?.length > 0 && !store.isFetching"
							...
							<Spinner v-else-if="store.isFetching" class="flex p-2 items-center justify-center" />
							<div class="p-2">
								<button id="saveview-button" data-modal-target="saveview-modal"
									data-modal-toggle="saveview-modal" class="primary-button-sm">
									<PlusIcon class="w-5 h-5 font-bold max-md:w-5 max-md:h-5" />
									Add new
								</button>
							</div>
						</div>
						<div id="saveview-modal" tabindex="-1" aria-hidden="true"
						...
						<button v-if="store.selectedRecords?.length > 0 &&
							tableConfig?.user_role?.delete
							" id="deleteSelectedRecordButton" data-modal-toggle="deleteSelectedRecord"
						...
						<RecordsDeleteSelected v-if="store.selectedRecords?.length > 0 &&
							tableConfig?.user_role?.delete
							" @delete-record="
		store.deleteRecord();
	tableKey++;
	" :length="store.selectedRecords.length" />
						...
						<button v-if="tableConfig?.user_role?.insert" id="addRecordButton" data-modal-toggle="addRecord"
						...
						<RecordsAdd :headers="tableConfig.headers" :useStore="tableConfig.store"
		...
		<RecordsTable v-if="!store.isFetching" :key="tableKey" :TABLE_DATA="store.recordsData" :tableConfig="tableConfig"
